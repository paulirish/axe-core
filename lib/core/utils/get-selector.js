...
function checkSelectorCache(elm) {
  if (!selectorCache) return false;
  return selectorCache.has(elm);
}

function getSelectorCacheValue(elm) {
  return selectorCache.get(elm);
}

function setSelectorCacheValue(elm, value) {
  if (!selectorCache) return;
  selectorCache.set(elm, value);
}
 ...
 */
axe.utils.getSelector = function createUniqueSelector (node, options = {}) {
  if (!node) {
    return '';
  }
  if (checkSelectorCache(node)) {
    return getSelectorCacheValue(node);
  }
  let elm = node;
  let doc = (elm.getRootNode && elm.getRootNode()) || document;
  let ret;
  if (doc.nodeType === 11) { // DOCUMENT_FRAGMENT
    ...
    stack.push({elm: elm, doc: doc});
    ret = stack.reverse().map((comp) => {
      return generateSelector(comp.elm, options, comp.doc);
    });
  } else {
    ret = generateSelector(elm, options, doc);
  }
  setSelectorCacheValue(node, ret);
  return ret;
};
